...
@Autonomous(name="Qick_auto", group="Robot")
public class Qick_auto extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        arm_mover(200,.94,200.0);

        // arm_mover_and_drive (150,-10,10,200.0);
        ...
        encoderDrive(DRIVE_SPEED, -3, 3, 5.0);//D


        arm_mover(-100,1.2,5.0);

        encoderDrive(DRIVE_SPEED, -2, 2, 5.0);//D


        graber.setPosition(0);
        graber2.setPosition(1);

        arm_mover(150,.96,5.0);
    ...
    public void arm_mover (
        ...
        if (opModeIsActive()) {
            ...
            while (currentVoltage != volts && touchSensor.getState() == true && volts != 10) {
                ...
                int ticks_to_move_arm = (int) (1000 * pot_difference);
    ...
    public void arm_mover_and_drive (
            double liftInches,
            double leftInches,
            double rightInches,
            double timeoutS
        ...
        // Ensure that the OpMode is still active
        if (opModeIsActive()) {
            int newfrontLeftTarget = fleftDrive.getCurrentPosition() + (int)(leftInches * COUNTS_PER_INCH);
            int newfrontRightTarget = frightDrive.getCurrentPosition() + (int)(rightInches * COUNTS_PER_INCH);
            int newbackLeftTarget = bleftDrive.getCurrentPosition() + (int)(leftInches * COUNTS_PER_INCH);
            int newbackRightTarget = brightDrive.getCurrentPosition() + (int)(rightInches * COUNTS_PER_INCH);
            ...
            lift.setPower(1);
            fleftDrive.setVelocity(Math.abs(1250));
            frightDrive.setVelocity(Math.abs(1250));
            bleftDrive.setVelocity(Math.abs(1250));
            brightDrive.setVelocity(Math.abs(1250));
            ...
            while (opModeIsActive() &&
                    ...
                    (fleftDrive.isBusy()) &&
                    (bleftDrive.isBusy())) {
                telemetry.addData("Running to", newbackRightTarget );
                telemetry.addData("the intialized position of  backRight", brightDrive.getCurrentPosition());

                telemetry.addData("Running to", currentarmposition );
                telemetry.addData("the intialized position of  lift", lift.getCurrentPosition());
    ...
    public void turning(double heading){
        ...
        while ((int)pos.h != (int)heading){
            ...
            if(pos.h > heading){
                ...
                fleftDrive.setVelocity(-TURN_SPEED);
                frightDrive.setVelocity(-TURN_SPEED);
                bleftDrive.setVelocity(-TURN_SPEED);
                brightDrive.setVelocity(-TURN_SPEED);
            }

            telemetry.addData("ext ext_pos_h:",  pos.h);

            pos = myOtos.getPosition();
            telemetry.addData("speed", TURN_SPEED );
            telemetry.addData("new ext_pos_h:",  heading);
            telemetry.update();
